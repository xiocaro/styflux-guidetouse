...
var fs=require('fs'),
	os=require('os'),
	path=require('path'),
	monocle=require("../node_modules/monocle")(),
...
(function(styflux){
		...
		var _this=this,
			...
			interpret={
				...
				"default":function(){ //Identifica que debe ir al inicio del documento
					json.prepend=json.prepend+"@import '"+rout+"'\n";
				}
			};
		if(arrs!=null){
		...
		if(json.constructor==Object){
			...
			for(var index in arr){
				...
				if(typeof route[index]!="undefined"){
					...
					rsp=rsp+index;
					(typeof json[index]=="string")?callback(rsp+"/"+json[index],json[index]):_this.createRoutes({"json":json[index],"result":rsp,"callback":callback});
				}
			}
			...
			(function(dir,ignore,imports){
				...
				_this.mapFiles(dir,compile.ext,ignore,function(file,name){ //Devuelve rutas de archivos
					_this.tmpFile(file,function(data,routeTmp){ //Devuelve la lectura del archivo y la ruta temporal
						_this.appendFile(file,vars,imports,data,function(content){ //Agrega las variables al inicio o al final
							_this.compile(file,compile,function(){ //Compila
	...
	styflux.watchProject=function(routes){
		var _this=this, 
			config=JSON.parse(fs.readFileSync(routes + "/config/config.json")),
			compile=config.compile || {"ext":".styl","cmd":"stylus -c"},
			file=_this.arrmapFiles(routes,compile.ext,[routes+"/library",routes+"/node_modules",routes+"/result"]),
			flag=true;
		console.log('  \033[90mEscuchando...\033[0m');
		monocle.watchFiles({
			files: file,
			listener: function(file) {
				if(flag){
					flag=false;
					child = exec("node "+routes+"/init.njs", function (error, stdout, stderr) {
						if (error != null) {console.log(error);}
						console.log('  \033[90mRenderizo Styflux\033[0m');
						flag=true;
					});
				}							
			}
